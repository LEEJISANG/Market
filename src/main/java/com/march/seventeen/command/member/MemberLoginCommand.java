...
public class MemberLoginCommand implements MemberCommand {
	...
	public Map<String, Object> execute(SqlSession sqlSession, Model model) {
		...
		if(memberDao.login(m_email, m_pw) != null) {
			resultMap.put("loginResult", 1);
			session.setAttribute("loginDto", memberDao.login(m_email, m_pw));
			System.out.println("로그인한 회원 DB: " + memberDao.login(m_email, m_pw));
		}else {
